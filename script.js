...
function create() {
  ...
  target = this.physics.add.sprite(
      Phaser.Math.Between(width * 0.7, width - 100),
      Phaser.Math.Between(height * 0.2, height * 0.8),
      "target"
  );
  target.setVelocityX(-200);
  target.setScale(0.32);
...
function update() {
  ...
  if (target.x >= width - target.width / 2) {
      target.setVelocityX(-200);
...
function handleShoot(pointer) {
  const width = game.config.width;
  const height = game.config.height;
  const arrowSpeed = 500;

  arrow.visible = true;
  arrow.setPosition(bow.x, bow.y);
  arrow.setRotation(bow.rotation- 30 );

  if (this.input.pointer1.isDown && this.input.pointer2.isDown) {
      const targetX = (this.input.pointer1.x + this.input.pointer2.x) / 2;
      this.physics.moveTo(arrow, targetX, 0, arrowSpeed);
  } else {
      this.physics.moveTo(arrow, pointer.x, pointer.y, arrowSpeed);
  }
}
...
function handleHit(arrow, target) {
  ...
  // Set a fixed position for the cheer girl
  cheerGirl.setPosition(width - cheerGirl.displayWidth / 2 - 50, height - cheerGirl.displayHeight / 2 - 50);
  cheerGirl.setVisible(true);

  // Jiggle the cheer girl and hide after the animation
  this.tweens.add({
      targets: cheerGirl,
      y: cheerGirl.y - 20,
      duration: 200,
...
function repositionTarget() {
  ...
  target.setPosition(
      Phaser.Math.Between(target.width / 2, width - target.width / 2),
      Phaser.Math.Between(target.height / 2, height - target.height / 2)
  );
